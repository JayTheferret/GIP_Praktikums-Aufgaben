...
bool update_boxes(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++) {
		...
		boxes[i].y += boxes[i].delta_y; //1.


		if ((boxes[i].y + box_size) > gip_win_size_y)//2.
			return(false);//false wenn rand unten erreicht
...
int main()
{
	...
	while (keep_going && gip_window_not_closed())
	{
		...
		keep_going = update_boxes(boxes);//unterbrechung
	}
	return 0;
}